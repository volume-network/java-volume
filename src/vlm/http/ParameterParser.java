...
final class ParameterParser {
    ...
    static long getAccountId(HttpServletRequest req) throws ParameterException {
        String recipientValue = Convert.emptyToNull(req.getParameter(ACCOUNT_PARAMETER));
        if (recipientValue == null || Parameters.isZero(recipientValue)) {
    ...
    static long getPoolId(HttpServletRequest req) throws ParameterException {
        String recipientValue = Convert.emptyToNull(req.getParameter(POOL_PARAMETER));
        if (recipientValue == null || Parameters.isZero(recipientValue)) {
            throw new ParameterException(MISSING_POOL);
        }
        long recipientId;
        try {
            recipientId = Convert.parseAccountId(recipientValue);
        } catch (RuntimeException e) {
            throw new ParameterException(INCORRECT_POOL);
        }
        if (recipientId == 0) {
            throw new ParameterException(INCORRECT_POOL);
        }
    ...
    static long getAccountIdByDefault(String account) {
        long accountId = 0;
        String accountValue = Convert.emptyToNull(account);
        if (accountValue == null || Parameters.isZero(accountValue)) {
            accountId = 0;
        } else {
            try {
    ...
    static String getSortSQL(HttpServletRequest req) {
        StringBuffer sortStrBf = new StringBuffer();
        String sortStr = "";
        String fullSorts = req.getParameter(PAGE_SORT_PARAMETER);
        try {
            String fullSortArray[] = fullSorts.split(";");
            if (fullSortArray.length > 0) {
                sortStrBf.append("order by ");
            }
            for (String fullSort : fullSortArray) {
                String sortArray[] = fullSort.split(",", 2);
                ...
                sortStrBf.append(sortType).append(" ").append(sortValue).append(",");
            }
        } catch (Exception e) {
            return sortStr;
        }
        System.out.printf("getSort:%s\n", fullSorts);

        sortStr = sortStr.toString();
        if (sortStr.length() > 0) {
            sortStr.substring(0, sortStr.length() - 2);
        }
        System.out.printf("getSort result:%s\n", sortStr);
        return sortStr;
    }

    static String getPageSQL(HttpServletRequest req) {
        StringBuffer pageStr = new StringBuffer("limit");
        int limit = getLimit(req);
        int page = getPage(req);
        int start = (page - 1) * limit;

        pageStr.append(" ").append(start).append(",").append("limit");
        System.out.printf("getPageString: %s\n", pageStr);
        return pageStr.toString();
    }

    // limit: default 10, (0 - 100]
    static int getLimit(HttpServletRequest req) {
        int limit;
        try {
            limit = Integer.parseInt(req.getParameter(PAGE_SIZE_PARAMETER));
            if (limit <= 0) {
                limit = 10;
            }
            if (limit > 1024) {
    ...
    //page: [1-~)
    static int getPage(HttpServletRequest req) {
        int page;
        try {
            page = Integer.parseInt(req.getParameter(PAGE_INDEX_PARAMETER));
            if (page < 1) {
                page = 1;
            }
        } catch (NumberFormatException e) {

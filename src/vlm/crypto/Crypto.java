...
public final class Crypto {
    ...
    public static byte[] getPublicKey(String secretPhrase) {
        ...
        // logger.info("### secretPhrase byte:[{}]", Convert.toBytes(secretPhrase));
        // logger.info("### secretPhrase sha256 digest:[{}]", Crypto.sha256().digest(Convert.toBytes(secretPhrase)));
        byte[] publicKeyHash = Crypto.sha256().digest(publicKey);
        long id = Convert.fullHashToId(publicKeyHash);
        // logger.info("### secretPhrase uid:[{}] - unsigned uid:[{}]", id, Convert.toUnsignedLong(id));
        // logger.info("### getPublicKey, secretPhrase:[{}], publicKey:[{}], HexPublicKey:[{}]", secretPhrase, publicKey, Convert.toHexString(publicKey));

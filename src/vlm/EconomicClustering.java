...
public final class EconomicClustering {
    ...
    public boolean verifyFork(Transaction transaction) {
        try {
            //if (Volume.getFluxCapacitor().isActive(FeatureToggle.DIGITAL_GOODS_STORE)) {
            if (Volume.getFluxCapacitor().isActive(FeatureToggle.DIGITAL_GOODS_STORE)) { ///  enable by default ???
                return true;
            }

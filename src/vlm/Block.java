...
public class Block {
    ...
    Block(int version, int timestamp, long previousBlockId, long totalAmountNQT, long totalFeeNQT,
          ...
          long nonce, byte[] blockATs, int height) throws VolumeException.ValidationException {

        if (payloadLength > Volume.getFluxCapacitor().getInt(FluxInt.MAX_PAYLOAD_LENGTH, height) || payloadLength < 0) {
        ...
        if (transactions != null) {
            ...
            if (blockTransactions.get().size() > (Volume.getFluxCapacitor().getInt(FluxInt.MAX_NUMBER_TRANSACTIONS, height))) {
                throw new VolumeException.NotValidException(
            ...
            for (Transaction transaction : this.blockTransactions.get()) {
                if (transaction.getId() <= previousId && previousId != 0) {
                    System.out.printf("previousId:%s, curID:%s", previousId, transaction.getId());
                    throw new VolumeException.NotValidException("Block transactions are not sorted!");
                }
    ...
    public Block(int version, int timestamp, long previousBlockId, long totalAmountNQT, long totalFeeNQT, int payloadLength, byte[] payloadHash, byte[] generatorPublicKey, byte[] generationSignature, byte[] blockSignature, byte[] previousBlockHash, BigInteger cumulativeDifficulty, long baseTarget,
                 long nextBlockId, int height, Long id, long nonce, byte[] blockATs) throws VolumeException.ValidationException {
    ...
    static Block parseBlock(JsonObject blockData, int height) throws VolumeException.ValidationException {
        try {
            ...
            for (JsonElement transactionData : transactionsData) {
                ...
                if (transaction.getSignature() != null) {
                    if (blockTransactions.put(transaction.getId(), transaction) != null) {
                        throw new VolumeException.NotValidException("Block contains duplicate transactions: " + transaction.getStringId());
                    }
                }
            }

            byte[] blockATs = Convert.parseHexString(JSON.getAsString(blockData.get("blockATs")));
            long forgeReward = JSON.getAsLong(blockData.get("forgeReward"));
            long poolId = Convert.parseUnsignedLong(JSON.getAsString(blockData.get("poolId")));
            Block block = new Block(version, timestamp, previousBlock, totalAmountNQT, totalFeeNQT,
    ...
    private TransactionDb transactionDb() {
        return Volume.getDbs().getTransactionDb();
    }
    ...
    public long getForgeReward() {
        return this.forgeReward;
    }

    public void setForgeReward(long reward) {
        this.forgeReward = reward;
    }
    ...
    public JsonObject getJsonObject() {
        ...
        json.addProperty("forgeReward", Convert.toUnsignedLong(forgeReward));
        json.addProperty("poolId", Convert.toUnsignedLong(poolId));
        return json;
    }

    public byte[] getBytes() {
        ByteBuffer buffer = ByteBuffer.allocate(4 + 4 + 8 + 4 + (version < 3 ? (4 + 4) : (8 + 8)) + 4
                + 32 + 32 + (32 + 32) + 8 + (blockATs != null ? blockATs.length : 0) + 8 + 8 + 64);
        ...
        if (version < 3) {
            buffer.putInt((int) (totalAmountNQT / Constants.ONE_COIN));
            buffer.putInt((int) (totalFeeNQT / Constants.ONE_COIN));
        } else {
        ...
        buffer.putLong(0); // reserve for forgeReward
    ...
    void sign(String secretPhrase) {
        ...
        setPoolId(Account.getId(Crypto.getPublicKey(secretPhrase)));
        blockSignature = new byte[64];
        byte[] data = getBytes();
        byte[] data2 = new byte[data.length - 64];
        System.arraycopy(data, 0, data2, 0, data2.length);
        blockSignature = Crypto.sign(data2, secretPhrase);
        // logger.info("block sign, id:[{}], publicKey:[{}], sign:[{}], data:[{}]", getId(), Convert.toHexString(Crypto.getPublicKey(secretPhrase)), Convert.toHexString(blockSignature), Convert.toHexString(data2));
    }

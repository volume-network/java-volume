...
import vlm.schema.tables.records.EscrowDecisionRecord;
import vlm.schema.tables.records.EscrowRecord;
...
import static vlm.schema.Tables.ESCROW;
import static vlm.schema.Tables.ESCROW_DECISION;

public class SqlEscrowStore implements EscrowStore {
    private final DbKey.LongKeyFactory<Escrow> escrowDbKeyFactory = new vlm.db.sql.DbKey.LongKeyFactory<Escrow>("id") {
        @Override
    ...
    private final vlm.db.sql.DbKey.LinkKeyFactory<Escrow.Decision> decisionDbKeyFactory =
            new vlm.db.sql.DbKey.LinkKeyFactory<Escrow.Decision>("escrow_id", "account_id") {
                @Override
    ...
    private void saveDecision(DSLContext ctx, Escrow.Decision decision) {
        ...
        decisionRecord.setHeight(Volume.getBlockchain().getHeight());
    ...
    @Override
    public vlm.db.sql.DbKey.LinkKeyFactory<Escrow.Decision> getDecisionDbKeyFactory() {
    ...
    private void saveEscrow(DSLContext ctx, Escrow escrow) {
        ...
        escrowRecord.setHeight(Volume.getBlockchain().getHeight());

...
public abstract class VersionedEntitySqlTable<T> extends EntitySqlTable<T> implements VersionedEntityTable<T> {
    ...
    VersionedEntitySqlTable(String table, TableImpl<?> tableClass, DbKey.Factory<T> dbKeyFactory, DerivedTableManager derivedTableManager) {
        super(table, tableClass, dbKeyFactory, true, derivedTableManager);
    }

    static void rollback(final String table, final TableImpl tableClass, final int height, final vlm.db.sql.DbKey.Factory dbKeyFactory) {
        ...
        try (DSLContext ctx = Db.getDSLContext()) {
            ...
            List<vlm.db.sql.DbKey> dbKeys = new ArrayList<>();
            try (ResultSet toDeleteResultset = selectForDeleteQuery.fetchResultSet()) {
                while (toDeleteResultset.next()) {
                    dbKeys.add((vlm.db.sql.DbKey) dbKeyFactory.newKey(toDeleteResultset));
                }
            }
            ...
            for (vlm.db.sql.DbKey dbKey : dbKeys) {
    ...
    static void trim(final String table, final TableImpl tableClass, final int height, final vlm.db.sql.DbKey.Factory dbKeyFactory) {
        if (!Db.isInTransaction()) {
            throw new IllegalStateException("Not in transaction");
        }
        ...
        try (ResultSet rs = selectMaxHeightQuery.fetchResultSet()) {
            ...
            while (rs.next()) {
                vlm.db.sql.DbKey dbKey = (vlm.db.sql.DbKey) dbKeyFactory.newKey(rs);
    ...
    public boolean delete(T t) {
        ...
        try (DSLContext ctx = Db.getDSLContext()) {
            ...
            countQuery.addConditions(tableClass.field("height", Integer.class).lt(Volume.getBlockchain().getHeight()));

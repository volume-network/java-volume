...
public class AT_API_Platform_Impl extends AT_API_Impl {
    ...
    private static Long findTransaction(int startHeight, int endHeight, Long atID, int numOfTx, long minAmount) {
        return Volume.getStores().getAtStore().findTransaction(startHeight, endHeight, atID, numOfTx, minAmount);
    }

    private static int findTransactionHeight(Long transactionId, int height, Long atID, long minAmount) {
        return Volume.getStores().getAtStore().findTransactionHeight(transactionId, height, atID, minAmount);
    }
    ...
    public void put_Last_Block_Hash_In_A(AT_Machine_State state) {
        ...
        b.put(Volume.getBlockchain().getBlockAtHeight(state.getHeight() - 1).getBlockHash());
    ...
    public long get_Type_for_Tx_in_A(AT_Machine_State state) {
        ...
        Transaction tx = Volume.getBlockchain().getTransaction(txid);
    ...
    public long get_Amount_for_Tx_in_A(AT_Machine_State state) {
        ...
        if ((tx.getMessage() == null || Volume.getFluxCapacitor().isActive(FeatureToggle.AT_FIX_BLOCK_2, state.getHeight())) && state.minActivationAmount() <= tx.getAmountNQT()) {
    ...
    public long get_Random_Id_for_Tx_in_A(AT_Machine_State state) {
        ...
        bf.put(Volume.getBlockchain().getBlockAtHeight(blockHeight - 1).getGenerationSignature());
        bf.putLong(tx.getId());
    ...
    public void put_Last_Block_Generation_Signature_In_A(AT_Machine_State state) {
        ...
        b.put(Volume.getBlockchain().getBlockAtHeight(state.getHeight() - 1).getGenerationSignature());
    ...
    public long get_Current_Balance(AT_Machine_State state) {
        if (!Volume.getFluxCapacitor().isActive(FeatureToggle.AT_FIX_BLOCK_2, state.getHeight())) {
            return 0;
        }

...
INSTALLATION & UPGRADING:
  ...
  upgrade                       Upgrade the config files to VLM format
...
configurations files. If you have
  conf/vlm.properties.MainNet
  conf/vlm.properties.TestNet
  conf/vlm.properties.LocalDev
you can activate your MainNet config with "volume.sh switch MainNet"
...
function upgrade_conf () {
    ...
    if [ -r $NXT_CFG_NAME ]
    then
        BRS_CFG_NAME="${NXT_CFG_NAME//nxt/vlm}"
        ...
        BRS="${BRS//nxt\.debugTraceLog/vlm.debugTraceLog}"
...
function macos_dependency_install {
    ...
    echo "[+] Installing VLM wallet dependencies for macOS"
...
if [[ $# -gt 0 ]] ; then
    case "$MY_CMD" in
        ...
        "compile")
            ...
            if hash mvn 2>/dev/null; then
                ...
                cp dist/tmp/volume.jar .
                cp dist/tmp/volume.jar ./volnode/
                echo a .zip file has been built for distribution in dist/, its contents are in dist/tmp
                echo Nevertheless, now you can start the wallet with ./volume.sh
            else
            ...
            ;;
        "h2shell")
            java -cp volume.jar org.h2.tools.Shell
            ;;
        "import")
            ...
            if [[ $REPLY =~ ^[Yy]$ ]]; then
                if [[ $MY_ARG == "mariadb" ]]; then
                    if [[ -f $CONF_FILE ]]
                    then
                        read -p "Detected a previous database config(conf/vlm.properties). Do you want to use the config? " -n 1 -r
                        echo
                        if [[ $REPLY =~ ^[Yy]$ ]]
                        then
                            useCurrentConfig="1"
                    ...
                    if exists_or_get brs.h2.zip ; then
                        mkdir -p "$MY_DIR/volume_db"
                        rm -f volume_db/burst.trace.db
                        if unzip brs.h2.zip ; then
                            if mv burst.mv.db "$MY_DIR/volume_db"; then
            ...
            ;;
        "switch")
            CONF_BASE=conf/vlm.properties         # our symlink
            CONF_TGT=vlm.properties.$MY_ARG       # target of our symlink
    ...
    nohup java $BRS_DEVSTART -cp volume.jar:conf vlm.Volume &
    #java $BRS_DEVSTART -cp volume.jar:conf vlm.Volume
fi
